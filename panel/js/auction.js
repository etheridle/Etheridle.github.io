...
function getTodayLobby() {
    ...
    mainContract.xfLobby(currentDay).call({
        ...
        $('.fi-4')[0].innerHTML = ((calcDaysLobbyPayout(currentDay) / DESI) / daysTotalUsersEntry).toFixed(2)
    })

    let totalUserEntryTRON = 0
    ...
    mainContract.xfLobbyMembers(currentDay, user.address).call({
        ...
        for (var i = 0; i < res.tailIndex; i++) {
            mainContract.xfLobbyEntry(user.address, currentDay, i).call({
                ...
                totalUserEntryTRON += parseFloat(tronWeb.fromSun(res.rawAmount._hex))
                $('.fi-8')[0].innerHTML = abbreviate_number(totalUserEntryTRON, 2)
                $('.fi-6')[0].innerHTML = abbreviate_number(((calcDaysLobbyPayout(currentDay) / DESI) * totalUserEntryTRON / daysTotalUsersEntry), 2)
                ...
                //     $('.fi-7')[0].innerHTML = abbreviate_number(((daysTotalUsersEntry2 * 0.95) * totalUserEntryTRON / daysTotalUsersEntry), 2)
                // })
    ...
    mainContract.xfLobby(currentDay).call({
        ...
        daysTotalUsersEntry = parseFloat(tronWeb.fromSun(res))
        $(`.fi-10`)[0].innerHTML = abbreviate_number(daysTotalUsersEntry, 2)
    })
...
function getLobbyData(day) {
    ...
    mainContract.xfLobby(day).call({
        ...
        daysTotalUsersEntry += parseFloat(tronWeb.fromSun(res))
        $(`.fi-4-day-${day}`)[0].innerHTML = ((calcDaysLobbyPayout(day) / DESI) / daysTotalUsersEntry).toFixed(2)
        $(`.fi-5-day-${day}`)[0].innerHTML = "closed"
        $(`.fi-10-day-${day}`)[0].innerHTML = abbreviate_number(daysTotalUsersEntry, 2)
    })
    ...
    let totalUserEntryTRON = 0
    // how many times entered current day's lobby
    mainContract.xfLobbyMembers(day, user.address).call({
        shouldPollResponse: true,
    }).then(res => {
        for (var i = 0; i < res.tailIndex; i++) {
            mainContract.xfLobbyEntry(user.address, day, i).call({
                shouldPollResponse: true,
            }).then(res => {
                totalUserEntryTRON += parseFloat(tronWeb.fromSun(res.rawAmount._hex))
                $(`.fi-8-day-${day}`)[0].innerHTML = abbreviate_number(totalUserEntryTRON, 2)
                $(`.fi-6-day-${day}`)[0].innerHTML = abbreviate_number(((calcDaysLobbyPayout(day) / DESI) * totalUserEntryTRON / daysTotalUsersEntry), 2)
