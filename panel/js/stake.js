        ...
        <div class="intro-y">
            <div class="${activeRow} row-body inbox__item inline-block sm:block text-gray-700 bg-gray-100 border-b border-gray-200"
                ...
                <div class="flex px-5 py-3"
                    ...
                    <div class="w-64 sm:w-auto truncate"
                        style="width: 100vw; text-align:center; font-weight: 900;">
                        <span class="interest-tm-${ii} inbox__item--highlight" id="${stakedSuns}">--</span>
                    </div>
        ...
        for (var i = 0; i < stakedDays; i++) {
            DaysData.forEach(item => {
                if (item.day == lockedDay + i) {
                    addUpDivs += (item.totalDividends * 0.95) * stakeShares / item.stakeSharesTotal
...
function calcInterest(item, item2, item3, stakeShares, startDay, endDay) {
    setTimeout(() => {
        ...
        for (var i = startDay; i < endDay; i++) {
            DaysData.forEach(item => {
                if (item.day == i) {
                    interest += item.payoutTotal * stakeShares / item.stakeSharesTotal
                }
            })
        }
        ...
        // $(`.${item3}`)[0].innerHTML = `${abbreviate_number(parseFloat($(`.${item2}`)[0].id) + parseFloat($(`.${item3}`)[0].id) + interest / 100, 2)} T2X`
    }, 3000)
}


//a.slice(0, a.indexOf(" T2X"))
        ...
        <div class="intro-y">
            <div class="${activeRow} row-body inbox__item inline-block sm:block text-gray-700 bg-gray-100 border-b border-gray-200"
                ...
                <div class="flex px-5 py-3" style="padding-left: .0rem; padding-right: .0rem;">
                    ...
                    <div class="w-64 sm:w-auto truncate"
                        style="width: 270vw; text-align:center; font-weight: 900;">
                        <span class="inbox__item--highlight">${abbreviate_number(parseInt(item.payout) / DESI, 2)} T2X</span>
                    </div>

                    <div class="w-64 sm:w-auto truncate"
                        style="width: 140vw; text-align:center; font-weight: 900;">
